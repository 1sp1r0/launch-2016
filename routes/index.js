...
router.get('/', function(req, res, next) {
  ...
  res.render('index', { title: 'Huddle', content: 'Continuing to make teams less busy since 2016!' });
...
router.get('/slack/oauth', function(req, res){
	...
	var content = {
		...
		viewcontent: {
			title: 'Huddle', 
			content: "Was installed successfully! Woo Ho! Hop over Slack and start call up some huddles."
		}
	};
	...
	try {
		...
		if(req.query.code && req.query.state === SLACK_CLIENT_STATE){
			...
			request("[/slack/oauth]", options, function(error, body){
				...
				if(error) {
					...
					content.viewcontent.content = "Failed to install Huddle into the team. :( Please try again.";
		...
		content.content = "Oh Man! Somethings not right. Please try again later.";
